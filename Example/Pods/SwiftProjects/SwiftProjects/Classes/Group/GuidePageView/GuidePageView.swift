...
public class GuidePageView: UIView {
    ...
    public lazy var logtinButton: UIButton = {
        let btn = UIButton.init(type: .custom)
        btn.setTitle("进入简纸APP", for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        btn.setTitleColor(UIColor(red: 0.98, green: 0.58, blue: 0.01,alpha:1), for: .normal)
        btn.titleLabel?.sizeToFit()
//        btn.backgroundColor = UIColor(red: 0.98, green: 0.58, blue: 0.01,alpha:1)
        btn.addTarget(self, action: #selector(loginBtnClicked), for: .touchUpInside)
        
        btn.layer.cornerRadius = 7.5
        btn.layer.borderWidth = 0.5
        btn.layer.borderColor = UIColor(red: 0.98, green: 0.58, blue: 0.01,alpha:1).cgColor
        return btn
    }()
    /// 立即体验按钮
...
//            } else {                    // 其它图片
            ...
            view.backgroundColor = UIColor.red
                (view as! UIImageView).image = UIImage.init(named: name)
